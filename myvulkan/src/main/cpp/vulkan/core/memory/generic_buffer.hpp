...
class GenericBuffer {
   public:
    GenericBuffer(VulkanContext& context,
        ...
        mDeviceBuffer = std::make_unique<DeviceBuffer>(context, mSize, usage, properties);
    }

    void update(const T* data, size_t count, size_t offset = 0) {
        if (!data || count == 0) return;
        if (offset + count > mMaxCount) throw std::runtime_error("GenericBuffer::update: out of range");
