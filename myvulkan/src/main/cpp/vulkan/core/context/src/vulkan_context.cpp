...
VulkanContext::VulkanContext(AAssetManager* assetManager) {
    ...
    mGraphicsCommandPool = createCommandPool(
        mDevice.get(), mPhysicalDeviceBundle.queueFamilyIndex, vk::CommandPoolCreateFlagBits::eResetCommandBuffer);
    mTransientCommandPool = createCommandPool(
        mDevice.get(), mPhysicalDeviceBundle.queueFamilyIndex, vk::CommandPoolCreateFlagBits::eTransient);
...
vk::UniqueInstance VulkanContext::createVkInstance() {
    vk::ApplicationInfo appInfo{
        "BarberPole", VK_MAKE_VERSION(1, 0, 0), "NoEngine", VK_MAKE_VERSION(1, 0, 0), VK_API_VERSION_1_1};
